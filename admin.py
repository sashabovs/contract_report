...
@admin_app.route("/users/<id>", methods=["PUT"])
def edit_user(id):
    data = flask.request.get_json()
    ...
    session = sqlalchemy.orm.Session(db_utils.engine_writer)
    session.query(model.Users).filter(model.Users.id == id).update(
        {
            model.Users.id: data["user"]["id"],
            model.Users.full_name: data["user"]["full_name"],
            model.Users.job_title_id: (
                data["user"]["job_title_id"]
                if "job_title_id" in data["user"] and data["user"]["job_title_id"]
                else None
            ...
            model.Users.role: data["user"]["role"],
            model.Users.login: data["user"]["login"],
            model.Users.email: data["user"]["email"],
            model.Users.password: new_pass,
        }
    )
    session.commit()
